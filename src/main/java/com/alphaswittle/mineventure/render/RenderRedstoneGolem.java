...
import com.alphaswittle.mineventure.model.ModelGolem;

import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

@SideOnly(Side.CLIENT)
public class RenderRedstoneGolem extends RenderLiving {
	private static ResourceLocation golemTexture = new ResourceLocation("mineventure:textures/entity/golem/redstone_golem.png");
	private final ModelGolem golemModel;

	public RenderRedstoneGolem() {
		super(new ModelGolem(), 0.5F);
		golemModel = (ModelGolem) mainModel;
	}

	public void doRender(EntityRedstoneGolem golem, double d0, double d1, double d2, float f0, float f1) {
		super.doRender(golem, d0, d1, d2, f0, f1);
	}

	protected ResourceLocation getEntityTexture(EntityRedstoneGolem golem) {
		return RenderRedstoneGolem.golemTexture;
	}

	protected void rotateCorpse(EntityRedstoneGolem golem, float f0, float f1, float f2) {
	...
	protected void renderEquippedItems(EntityRedstoneGolem golem, float f0) {
	...
	public void doRender(EntityLiving golem, double d0, double d1, double d2, float f0, float f1) {
		this.doRender((EntityRedstoneGolem) golem, d0, d1, d2, f0, f1);
	}

	@Override
	protected void renderEquippedItems(EntityLivingBase golem, float f0) {
		this.renderEquippedItems((EntityRedstoneGolem) golem, f0);
	}

	@Override
	protected void rotateCorpse(EntityLivingBase golem, float f0, float f1, float f2) {
		this.rotateCorpse((EntityRedstoneGolem) golem, f0, f1, f2);
	}
	...
	protected ResourceLocation getEntityTexture(Entity golem) {
		return this.getEntityTexture((EntityRedstoneGolem) golem);
	}
