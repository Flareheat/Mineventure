...
public class RenderWoodGolem extends RenderLiving {
	private static ResourceLocation golemTexture = new ResourceLocation("mineventure:textures/entity/golem/wood_golem.png");
	private final ModelGolem golemModel;

	public RenderWoodGolem() {
		super(new ModelGolem(), 0.5F);
		golemModel = (ModelGolem) mainModel;
	}

	public void doRender(EntityWoodGolem golem, double d0, double d1, double d2, float f0, float f1) {
		super.doRender(golem, d0, d1, d2, f0, f1);
	}

	protected ResourceLocation getEntityTexture(EntityWoodGolem golem) {
		return RenderWoodGolem.golemTexture;
	}

	protected void rotateCorpse(EntityWoodGolem golem, float f0, float f1, float f2) {
	...
	protected void renderEquippedItems(EntityWoodGolem golem, float f0) {
	...
	public void doRender(EntityLiving golem, double d0, double d1, double d2, float f0, float f1) {
		this.doRender((EntityWoodGolem) golem, d0, d1, d2, f0, f1);
	}

	@Override
	protected void renderEquippedItems(EntityLivingBase golem, float f0) {
		this.renderEquippedItems((EntityWoodGolem) golem, f0);
	}

	@Override
	protected void rotateCorpse(EntityLivingBase golem, float f0, float f1, float f2) {
		this.rotateCorpse((EntityWoodGolem) golem, f0, f1, f2);
	}
	...
	protected ResourceLocation getEntityTexture(Entity golem) {
		return this.getEntityTexture((EntityWoodGolem) golem);
	}
