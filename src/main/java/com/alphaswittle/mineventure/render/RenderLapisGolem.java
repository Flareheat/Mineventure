...
public class RenderLapisGolem extends RenderLiving {
	private static ResourceLocation golemTexture = new ResourceLocation("mineventure:textures/entity/golem/lapis_golem.png");
	private final ModelGolem golemModel;

	public RenderLapisGolem() {
		super(new ModelGolem(), 0.5F);
		golemModel = (ModelGolem) mainModel;
	}

	public void doRender(EntityLapisGolem golem, double d0, double d1, double d2, float f0, float f1) {
		super.doRender(golem, d0, d1, d2, f0, f1);
	}

	protected ResourceLocation getEntityTexture(EntityLapisGolem golem) {
		return RenderLapisGolem.golemTexture;
	}

	protected void rotateCorpse(EntityLapisGolem golem, float f0, float f1, float f2) {
	...
	protected void renderEquippedItems(EntityLapisGolem golem, float f0) {
	...
	public void doRender(EntityLiving golem, double d0, double d1, double d2, float f0, float f1) {
		this.doRender((EntityLapisGolem) golem, d0, d1, d2, f0, f1);
	}

	@Override
	protected void renderEquippedItems(EntityLivingBase golem, float f0) {
		this.renderEquippedItems((EntityLapisGolem) golem, f0);
	}

	@Override
	protected void rotateCorpse(EntityLivingBase golem, float f0, float f1, float f2) {
		this.rotateCorpse((EntityLapisGolem) golem, f0, f1, f2);
	}
	...
	protected ResourceLocation getEntityTexture(Entity golem) {
		return this.getEntityTexture((EntityLapisGolem) golem);
	}
